...
#define Hex2Int(c) ((int)((int)p <='9' ? (int)p - '0' : (int)p <='F' ? (int)p -'A' + 10 : (int)p - 'a' + 10))
///
...
void OutlineCharacter(unsigned char *bitmap, int width, int height)
{
    ...
    outline = (unsigned char*)alloca(((width + 7) / 8) * height);
...
void ReadBdf(FILE * bdf, FILE * out, const char *name)
{
    ...
    //	Allocate tables
    //
    width_table = (unsigned int*)malloc(chars * sizeof(*width_table));
    if (!width_table) {
	fprintf(stderr, "Out of memory\n");
	exit(-1);
    }
    encoding_table = (unsigned int*)malloc(chars * sizeof(*encoding_table));
    if (!encoding_table) {
	fprintf(stderr, "Out of memory\n");
	exit(-1);
    }
     ...
     */
    bitmap =
	(unsigned char*)malloc(((fontboundingbox_width + 7) / 8) * fontboundingbox_height);
    if (!bitmap) {
	fprintf(stderr, "Out of memory\n");
	exit(-1);
    }
